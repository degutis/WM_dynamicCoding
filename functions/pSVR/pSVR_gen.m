function pSVR_gen(experiment,subject,voxSelect,expandIPS)
...
wmNoonNo_fcaMatrix = cell(numel(sampleFiles),numel(my_rois_plus));
wmOronOr_fcaMatrix = cell(numel(sampleFiles),numel(my_rois_plus));
wmNoiseonNoise_fcaMatrix= cell(numel(sampleFiles),numel(my_rois_plus));
...
for sub = subject:subject
    ...
    for roi = 1:length(my_rois_plus)
        ...
        wm_noDist = mean(wm_noDist(5:end,:,:),1);
        wm_orDist = mean(wm_orDist(5:end,:,:),1);
        wm_noiseDist = mean(wm_noiseDist(5:end,:,:),1);
        
        [wmNoonNo_fcaMatrix(sub,roi),~] = runPSVR_runwise(wm_noDist,wm_noDist,wmTrials(:),wmTrials(:));               
        [wmOronOr_fcaMatrix(sub,roi),~] = runPSVR_runwise(wm_orDist,wm_orDist,wmTrials_orDist(:),wmTrials_orDist(:));               
        [wmNoiseonNoise_fcaMatrix(sub,roi),~] = runPSVR_runwise(wm_noiseDist,wm_noiseDist,wmTrials_noise(:),wmTrials_noise(:));               
        
        [wmOronMem_fcaMatrix(sub,roi),~] = runPSVR_runwise(wm_noDist,wm_orDist,wmTrials(:),wmTrials_orDist(:));        
        [wmNoiseonMem_fcaMatrix(sub,roi),~] = runPSVR_runwise(wm_noDist,wm_noiseDist,wmTrials(:),wmTrials_noise(:));

    end
end

save(fullfile(results_new,['gen_runwise_PSVR_',num2str(subject),'.mat']),'wmNoonNo_fcaMatrix', 'wmOronOr_fcaMatrix','wmNoiseonNoise_fcaMatrix','wmOronMem_fcaMatrix','wmNoiseonMem_fcaMatrix')
