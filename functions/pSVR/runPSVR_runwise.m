function [genMatrix_fca, genMatrix_bfca] = runPSVR_runwise(data_train, data_test,labels_train,labels_test)
...
%           reconstruction
    ...
    genMatrix_fca = zeros([1,3]);
    genMatrix_bfca = zeros([1,3]);
                
    for session = 1:3
    % get data dimensions
        data_train_c = data_train(1,:,trainIndex(:,session));
        data_train_c = squeeze(data_train_c)';
        data_test_c = data_test(1,:,testIndex(:,session));
        ...
        genMatrix_bfca(:,session) = bal_norm_circ_resp_dev(ang_y, true_labels, 'trapz').*100;
        genMatrix_fca(:,session) = avg_norm_circ_resp_dev(ang_y, true_labels).*100;
    end                
